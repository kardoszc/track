...
class Speed(object):
    ...
    def __init__(self,filename):
        self.MinTrackWidth = 1
        self.MaxTrackWidth = 10
    ...
    def get_line(self):
        ...
        for entity in self.dxf.entities:
            if self.find_layer_by_name(entity.layer).on and entity.dxftype == 'LINE':
                start = Point(entity.start[0], entity.start[1])
                end = Point(entity.end[0], entity.end[1])
                line = Line(start, end)
                lines.append(line)
            elif self.find_layer_by_name(entity.layer).on and entity.dxftype == 'POLYLINE':
    ...
    def draw_line(self, lines):
        ...
        size = [int(max(all_x)-min(all_x)),int(max(all_y)-min(all_y))]
